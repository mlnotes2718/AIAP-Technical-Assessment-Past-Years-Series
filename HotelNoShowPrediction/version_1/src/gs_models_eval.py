...
# Load the dataset
try:
    X_train_df = pd.read_csv('./data/X_train.csv', index_col='booking_id')
    y_train_df = pd.read_csv('./data/y_train.csv', index_col='booking_id')
except Exception as e:
...
for result in results:
    ...
    log_and_print(f"Best Score (Accuracy): {result['best_score']:.4f}")
