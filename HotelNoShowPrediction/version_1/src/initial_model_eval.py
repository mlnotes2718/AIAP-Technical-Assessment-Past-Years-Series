...
try:
    X_train_raw = pd.read_csv('./data/X_train.csv', index_col='booking_id')
    y_train_raw = pd.read_csv('./data/y_train.csv', index_col='booking_id')
except Exception as e:
...
for model_name in models_list:
    ...
    if model_name == 'logistic_regression':
        pipeline.set_params(logistic__max_iter = 10000)
    ...
    try:
        ...
        log_and_print('----------------------------------------------------------------')
        log_and_print(f'Accuracy score for training set: {pipeline.score(X_train, y_train)}')
        log_and_print(f'Accuracy score for cross validation set: {pipeline.score(X_cv, y_cv)}')
    ...
    log_and_print(f"Best Accuracy Score: {best_score:.4f}")
